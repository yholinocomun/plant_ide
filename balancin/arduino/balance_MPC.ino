/*  BALANCE MPC / LQR-PREDICTIVO - ESP32-S3 core 3.x
    u = -Kmpc * x  (estados MEDIDOS; sin observador).  Kmpc = Riccati horizonte N.
    Mismo esqueleto que el firmware LQR de referencia.  Teclas: space z o i g f
    Kmpc de balancin/python/sim_mpc.py */
#include <Arduino.h>
#include <Wire.h>
#include <math.h>
...
const float ANG_CAIDA=35.0,R2D=57.29578; const int U_DEAD=30,PWM_MIN=3,PWM_MAX=255;
// u = -(K0*x + K1*xd + K2*theta + K3*thetad)   [theta en rad]
...
void setup(){Serial.begin(115200);delay(400);
 ...
 Serial.println("MPC listo. space z o i g f");}
...
void loop(){
 if(Serial.available()){char c=Serial.read();
  ...
  else if(c=='f'){Serial.print("Kmpc set=");Serial.println(setpoint,2);} }
 ...
 float th=(ang-setpoint)/R2D, thd=gy/R2D;               // rad
 long enc=(encIzq+encDer)/2; float x=(enc/PPR)*2*M_PI*R_RUEDA, xd=(x-x_prev)/dt; x_prev=x;
 if(fabs(th*R2D)>ANG_CAIDA){parar();return;} if(!control_on){parar();return;}
 float u=-(Kmpc[2]*th+Kmpc[3]*thd);
 if(usePos) u+=-(Kmpc[0]*x+Kmpc[1]*xd);
