...
volatile long encIzq=0,encDer=0; float ang=0,x_prev=0,xh[4]={0,0,0,0},u_prev=0;
...
void loop(){
 if(Serial.available()){char c=Serial.read();
  ...
  else if(c=='f'){Serial.print("LQG set=");Serial.println(SETPOINT_FIJO,2);} }
 ...
 float theta_deg=(ang-ANGULO_CERO_FIJO)-SETPOINT_FIJO, theta_dot=gy, th=theta_deg/R2D;
